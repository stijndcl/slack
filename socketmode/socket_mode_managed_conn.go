...
import (
	...
	"github.com/stijndcl/slack/internal/backoff"
	"github.com/stijndcl/slack/internal/timex"
...
func (smc *Client) run(ctx context.Context, connectionCount int) error {
	...
	// Need to wait for runMessageReceiver to avoid panic described in https://github.com/stijndcl/slack/issues/1125
	wg.Add(1)
	go func() {
		defer wg.Done()
		defer cancel()
